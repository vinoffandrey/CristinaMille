		...
		document.addEventListener("click", function (e) {
			...
			if (buttonClose || !e.target.closest(`.${this.options.classes.popupContent}`) && this.isOpen) {
				...
				if (e.target.classList.contains('popup__close')) {
					const videoStop = e.target.nextElementSibling.querySelector('video');
					if (videoStop) {
						videoStop.pause();
						videoStop.currentTime = 0;
					}
				}
				const videoStopWrapper = e.target.querySelector('video');
				if (videoStopWrapper) {
					videoStopWrapper.pause();
					videoStopWrapper.currentTime = 0;
				}
				return;
			}
		}.bind(this));
